...
def plot3D_rgb_size(data,iord,bin_size=None,width=700,height=500,rgb_index=(3,4,5),client=False):
    ...
    # client mode
        ...
        del sep
#################################################
def plot3D_col(data,width=700,height=500,col_index=3,pointSize=1,client=False):
    ...
    for row in data[1:] :
        SOPI_color = cmap.get_color(int(row[col_index]))  # with midas_heat : icolor 0 is black, size-1 is white.
        r = SOPI_color.r()
        g = SOPI_color.g()
        b = SOPI_color.b()
        a = 1
