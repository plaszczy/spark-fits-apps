...
def autospec(c,L,bink=50,subtract_SN=True):
    ...
    if subtract_SN:
        ...
        return (array(kmean),array(psmean),array(stdps))


def crossspec(c,c2,L,bink=50):
    #c counts map (unscaled)
    #L en radian
    ...
    Ntot=c.sum()
    dens=Ntot/Ldeg**2/3600
    print("mean density on patch1={} gals/arcmin**2".format(dens))
    #rescale
    img=c/Nmean-1.
    ...
    Ntot=c2.sum()
    dens=Ntot/Ldeg**2/3600
    print("mean density on patch2={} gals/arcmin**2".format(dens))
    #rescale
    img2=c2/Nmean-1.
